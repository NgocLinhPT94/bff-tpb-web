...
export class ArticlesController {
  ...
  @ApiResponse({ status: 200, description: 'List of articles retrieved successfully' })
  ...
  async findAll(
    ...
    const result = await this.articlesService.findAll(query, request.isDraft);
  ...
  @ApiParam({ name: 'documentId', description: 'Article document ID' })
  @ApiResponse({ status: 200, description: 'Article retrieved successfully' })
  @ApiResponse({ status: 400, description: 'Invalid document ID', type: ErrorEnvelopeDto })
  @ApiResponse({ status: 404, description: 'Article not found', type: ErrorEnvelopeDto })
  ...
  async findOne(
    ...
    return createSuccessEnvelope(
      request,
      await this.articlesService.findOne(documentId, request.isDraft),
    );
  }
}
